...
class page_mesh():
    ...
    def weight_function(self,h,w):
        ...
        # w:よこ
        weight = 1#重みなし
        # weight = np.power(0.5,h/2160)#重み#1
    ...
    ##それぞれのデータをメッシュ化##
    ## メッシュ下の基準は、あるメッシュ1*1（1*1は要指定項目）において、self.mesh_bound以上占有している場合は１(上で単位を１としているので、)それ以外は0
    ## page_mesh が変える
    def make_mesh(self,is_binary = True):
        #1つのデータが１つの要素
        for ii,data in enumerate(self.data):
            #print  "eee",ii,len(self.data)
            mesh_dis = {}
            ...
            for y_min in range(self.default['H']):
                for x_min in range(self.default['W']):
                    #個数加算方式の場合
                    # tmp_area = self.weight_mesh[y_min][x_min] if mesh_dis['X'][x_min] * mesh_dis['Y'][y_min] > 0 else 0# "1"加算を重み付け*1で計算に変更。
                    tmp_area = 1 if mesh_dis['X'][x_min] * mesh_dis['Y'][y_min] > 0 else 0# "1"加算を重み付け*1で計算に変更。

                    #閾値以上個数加算方式の場合
                    #tmp_area =  1 if mesh_dis['X'][x_min] * mesh_dis['Y'][y_min] >= self.mesh_bound else 0
                    #専有面積加算方式の場合
                    #tmp_area =  mesh_dis['X'][x_min] * mesh_dis['Y'][y_min]
                    
                    mesh[y_min][x_min] += tmp_area
                    sum_area += tmp_area

                    
            np.append(self.area_data,sum_area)
            self.mesh += np.array(mesh)

        ### 各種統計量算出  ###
        (height, width) = self.mesh.shape
        self.mesh_vec_list_x = np.array([])
        self.mesh_vec_list_y = np.array([])
        for h in  range(height):
            for w  in  range(width):
                for cnt in range(int(self.mesh[h][w])):
                    np.append(self.mesh_vec_list_x,w)
                    np.append(self.mesh_vec_list_y,h)
        ...
        self.cov_mesh_area       = float(np.cov(self.area_data))#[0]#特殊処理
        self.mean_mesh_area      = np.mean(self.area_data)#[0]#特殊処理
        self.sum_mesh_area      = sum(self.area_data)#[0]#特殊処理
