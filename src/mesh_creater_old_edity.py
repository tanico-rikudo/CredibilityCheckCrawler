...
class page_mesh():
	...
	def convert_each_data(self,each_data):
		...
		if data['X'] > self.page_limit['W'] or data['Y'] > self.page_limit['H'] :
			return  None#どちらかの軸において全部が超えてたら除外全部０
		...
		#調整後→圧縮
		return  { item : float(data[item]) *self.compression_rate[item] for item in self.info_item }
	...
	def weight_function(self,h,w):
		...
		# w:よこ
		# weight = 1#重みなし
# 		weight = np.power(0.5,h/2160)#重み#1(２画面目最終で1/2に到達)
		weight = (0.5)**(h/2160.0) if h < self.page_limit_h_half else (0.5)**((self.page_limit['H']-h)/2160.0)  #重み#1(２画面目最終で1/2に到達)
 		#weight = 1 
	...
	def make_mesh(self,is_binary = True):
		...
		for data in self.data:
			...
			mesh = np.array([[ 0 for w in range(self.default['W'])] for h in range(self.default['H'])])

			for y_min in range(self.default['H']):
				for x_min in range(self.default['W']):
					#個数加算方式の場合
					#mesh[y_min][x_min] += self.weight_mesh[y_min][x_min] if mesh_dis['X'][x_min] * mesh_dis['Y'][y_min] > 0 else 0
					#閾値以上個数加算方式の場合
			...
			np.append(self.area_data,sum(mesh.flatten()))
